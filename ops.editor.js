...
const buildInThemes = {
	...
	"Card board": {
		...
		buttons: "#c4cfa1",
		textcolor: "#1f1f1f",
		channelTextColor: "#1f1f1f",
		pianoRollAndInputsBg: "#8b956d",
		tracksTextAndPianoWhite: "#1f1f1f",
		blackPianoAndTrackbg: "#8b956d",
		tracksBg: "#c4cfa1",
		inputAndDoneRename: "#8b956d",
		channelsColor: "#c4cfa1",
		editorBg: "#8b956d",
		patternBg: "#c4cfa1",
		highlight: "rgba(255,255,255,0.08)",
		done: "#c4cfa1",
		...
		if (!isMini) {
			...
			patternsToDraw.forEach(({ pat, px, pw, pattern }) => {
				...
				if (pattern.type === "mod") {
					...
					const isStep = pattern.config.isStep === true;
					...
					pts.forEach((p, i) => {
						...
						if (isStep && i > 0) {
							const prevAy = modDrawYStart + (1 - pts[i - 1].y) * modAvailableH;
							ctx.lineTo(ax, prevAy);
						}
...
function renderAutomationMenu(track, localX, activeMenuX, originalId = null) {
	...
	types.forEach(type => {
		...
		if (pat && pat.associated) {
			if (type === "Song Tempo" && pat.associated[0] === "SONG") {
				isSelected = true;
			} else if (type === "Instrument" && pat.category === "inst" && pat.associated[0] !== "SONG") {
				isSelected = true;
			} else if (type === "Effect" && pat.category === "fx" && pat.associated[0] !== "SONG") {
				isSelected = true;
			}
		}
		let row = new Node({
		...
		if (isSelected) {
			row.style.bg = blendHexWithRgba(theme.tracksBg, "rgba(200,255,200,0.4)")
		}
		row.onClick = () => {
			if (type === "Song Tempo") {
				instrumentBox.style.opacity = 1;
...
function renderInstanceSelection(track, category, localX, originalId = null) {
	...
	Object.keys(states).forEach(key => {
		...
		const pat = patterns[originalId];
		if (pat && pat.associated) {
			if (category === "instrument" && pat.associated[1] === key) {
				isSelected = true;
			}
			if (category === "effect" && pat.associated[0] === key) {
				isSelected = true;
			}
		}
		const row = new Node({
			...
			style: {
				...
				h: "40px",
				bg: isSelected ?
					blendHexWithRgba(theme.tracksBg, "rgba(200,255,200,0.4)") : theme.tracksBg,
				textleft: 10,
...
function renderParameterSelection(track, category, selectedStateKey, localX, originalId) {
	...
	Object.keys(automations).forEach(paramKey => {
		...
		let isSelected = false
		if (pat && pat.associated) {
			if (category === "instrument" && pat.associated[2] === paramKey) {
				isSelected = true;
			}
			if (category === "effect" && pat.associated[1] === paramKey) {
				isSelected = true;
			}
		}
		const row = new Node({
	...
	if (activeMenu) {
		...
		function renderPlugins() {
			...
			Object.entries(SongPlugins).forEach(([name, plugin]) => {
			 let snhfx= (plugin.parsed.meta.type)
				const row = new Node({
				...
				const label = new Node({
					value: name,
					style: { float: "left", w: "200px", texttop: 5, color: "window(theme.textcolor)" }
				});
				const fxOrSnhLabel = new Node({
					value: snhfx,
					style: { w: "200px", texttop: 15, color: "window(theme.textcolor)", opacity :0.6}
				});
				...
				const deleteBtn = new Node({
					...
					onClick: () => {
						...
						const tag = pluginToDelete.parsed.meta.name;
						const type = pluginToDelete.parsed.meta.type;
						const isInst = type === "instrument";
						if (isInst) {
							delete window.Instruments[tag];
						} else if (type === "fx") {
							delete window.Effects[tag];
						}
			...
			const addBtn = new Node({
				value: "Add Plugin",
				style: { float: "left", w: "100%", h: "40px", bg: "window(theme.buttons)",color:"window(theme.textcolor)" }
			});
			addBtn.onClick = () => {
       ...
       SongPlugins[synthName] = { 	data: base64,	source: text,	parsed: parsePlugin(text)};
							let metaType=SongPlugins[synthName].parsed.meta.type 
       const fileName = file.name.toLowerCase();
       
       if ((metaType === "instrument" && !fileName.endsWith(".opin")) ||
       (metaType === "fx" && !fileName.endsWith(".opfx"))) {
       throw new Error(`Unknown Format: `+metaType+" "+fileName);
       }
       delete window.Instruments[synthName];
       delete window.Effects[synthName];
							...
							addBtn.value = "LOADED: " + synthName;
						 	setTimeout(function() {
        addBtn.value = "Add Plugin";
        }, 1500)
						} catch (e) {
							addBtn.value = e ;setTimeout(function (){addBtn.value = "Add Plugin";
							},1500
							)
						}
					};
		...
		if (activeMenu && activeMenu.type == "select") {
			...
			activeMenu.opts.forEach((o, i) => {
				let highlight = false;
				const currentOption = activeMenu.orginalOpst?.[i];
				let displayValue = o;
				...
				if (highlight) {
					btn.style.bg = blendHexWithRgba(theme.tracksBg, "rgba(200,255,200,0.4)")
				}
					...
					const outputSelect = new Node({
						...
						onClick: (el) => {
							activeMenu = {
								type: "select",
								opts: [
									["Real-time Synth", () => { settings.buildMusicBeforePlay = 0;
										saveSettings() }],
									["Build Buffer", () => { settings.buildMusicBeforePlay = 1;
										saveSettings() }]
								]
							};
							menuBox.scroll.y = 0;
					...
					const pattSelect = new Node({
						...
						onClick: (el) => {
							activeMenu = {
								type: "select",
								opts: [
									["True", () => { settings.cleanPatterns = true;
										saveSettings() }],
									["False", () => { settings.cleanPatterns = false;
										saveSettings() }]
								]
							};
							menuBox.scroll.y = 0;
				...
				} else if (o === "Steps/Linear" || o === "Linear/Steps") {
					btn.onClick = () => {
						const pat = patterns[selectedPattern];
						console.log(selectedPattern)
						if (pat && pat.config) {
							pat.config.isStep = !pat.config.isStep;
						}
						draw();
						activeMenu = null;
						menuBox.style.opacity = 0;
					...
					btn.onClick = () => {
						...
						if (newId != null) {
							setTimeout(function() {
								...
								playNote(60, 4, patterns[newId].instrument, newId, lastVolume, true);
							}, 20);
						}
						activeMenu = null;
						menuBox.style.opacity = 0;
				...
				if (o === "Use Default Channel Color") {
					...
					btn.onClick = () => {
						...
						t.patterns.forEach(p => {
							const oldId = p[2];
							const oldPattern = patterns[oldId];
							const newId = "pat_" + Math.random().toString(36).substr(2, 9);
							const newPattern = JSON.parse(JSON.stringify(oldPattern));
							newPattern.playingMidi = new Set();
							newPattern.id = newId;
							patterns[newId] = newPattern;
							const newP = [...p];
							newP[2] = newId;
							newTrack.patterns.push(newP);
						});
						const i = tracks.indexOf(t);
						tracks.splice(i + 1, 0, newTrack);
						activeMenu = null;
						menuBox.style.opacity = 0;
						menuBoxBg.style.opacity = 0;
						draw();
		...
		dw.layers[1].forEach((item, i) => {
			...
			if (dw.inObj(item.hit.piano, x, y)) {
				...
				if (clickedPat && !bufferLoaded) {
					...
					let tPOSX = track.patterns[currentTrack.activePatternIdx][0]
					console.log(tPOSX * STEP_W, dawScrollX)
					const dawStepW = 4 * dawZoomX;
     const dawSteps = dawScrollX / dawStepW;
     const patStart = tPOSX;
     const offsetSteps = 0;
     const pScrollX2 = (dawSteps - patStart - offsetSteps) * STEP_W;
					if (pScrollX2 > 60) {
						pScrollX = Math.max(0, pScrollX2);
					} else {
						pScrollX = 0
					}
					pScrollY = 700;
		...
		if (x > 60) {
			if (pattern && ny > -1) {
				...
				if (!alreadyExists) {
					...
					playNote(ny, 4, pattern.instrument, selectedPattern, lastVolume, true);
				}
			}
		} else {
			pattern.playingMidi = pattern.playingMidi || new Set();
		...
		if (view === "DAW") {
			...
			if (ti !== null && !bufferLoaded) {
				...
				longPressTimer = setTimeout(() => {
					...
					if (pat && p.x > trackW && headerH > 0 && p.y >= piano.y && p.y <= piano.y + headerH && patterns[pat[2]].type == "mod") {
						const patObj = patterns[pat[2]];
						const stepToggle = patObj.config?.isStep ? "Steps/Linear" : "Linear/Steps";
						activeMenu = {
							x: p.x,
							track: track,
							pattern: pat,
							...
							opts: [
								...
								"Rename Clip",
								"Set Color",
								"Use Default Channel Color"
							]
						};
						menuBox.scroll.y = 0;
						selectedPattern = pat[2];
					} else if (pat && p.x > trackW && headerH > 0 && p.y >= piano.y && patterns[pat[2]].type !== "mod") {
						activeMenu = {
							x: p.x,
							track: track,
							pattern: pat,
...
function onUp(e) {
	...
	if (wasMovinNote) {
		...
		playNote(wasMovinNote.n.y, 4, wasMovinNote.instrument, selectedPattern, lastVolume, true);
	}
	if (!pointers.has(e.pointerId)) return
...
c.addEventListener("wheel", e => {
	...
	if (view === "DAW") {
		...
		if (e.shiftKey) {
			dawZoomY = clamp(dawZoomY - e.deltaY * 0.02, 20, 200);
		} else if (e.ctrlKey) {
			if (isPlaying) return;
		...
		if (e.shiftKey) {
			NOTE_H = clamp(NOTE_H - e.deltaY * 0.01, 6, 80);
			selectRect = null;
		} else if (e.ctrlKey) {
			STEP_W = clamp(STEP_W - e.deltaY * 0.01, 2, 120);
			selectRect = null;
		} else if (e.altKey) {
...
function init() {
	...
	fileBtn = new Node({
		...
		style: {
			...
			w: 'auto',
			h: "calc(window(window.NavH)-8)",
			size: "calc( (window(window.NavH)-8)/2)",
			ovx: "...",
			texttop: "calc( (window(window.NavH)-8)/4)",
			ov: 'hid',
			bg: "window(theme.buttons)",
	...
	addBtn = new Node({
		value: "+",
		style: {
			textleft: "calc(window(window.trackW)/2.5)",
			texttop: "calc((window(window.NavH)/2)-12)",
			pos: "abs",
			b: '0',
			l: '0',
			w: 'window(window.trackW)',
			h: "calc(window(window.NavH)-8)",
			ov: 'hid',
			bg: "window(theme.buttons)",
			highlight: "window(theme.highlight)",
	...
	playBtn = new Node({
		...
		style: {
			...
			highlight: "window(theme.highlight)",
			float: "left",
			color: "window(theme.playbtnColor)"
		}
	});
	ToStartBtn = new Node({
		value: "<",
	...
	instrumentPaste = new Node({ value: "PASTE", style: { pos: "abs", t: "5px", texttop: 5, r: 120, w: 'auto', h: '30px', bg: "window(theme.tracksBg)", color: "window(theme.textcolor)", opacity: 1 } });
	instrumentNav.add(instrumentPaste)
	instrumentCopy = new Node({ value: "COPY", style: { pos: "abs", t: "5px", texttop: 5, r: 55, w: 'auto', h: '30px', bg: "window(theme.tracksBg)", opacity: 1, color: "window(theme.textcolor)" } });
	instrumentNav.add(instrumentCopy)
	...
	instrumentOk = new Node({ value: "OK", style: { pos: "abs", t: "5px", texttop: 5, r: 5, w: 'auto', h: '30px', bg: "window(theme.done)", opacity: 1 } });
	instrumentNav.add(instrumentOk)
	instrumentOk.onClick = () => {
		instrumentBox.style.opacity = 0
		if (selectedPattern != null) {
			setTimeout(function() {
				patterns[selectedPattern].playingMidi = patterns[selectedPattern].playingMidi || new Set();
				patterns[selectedPattern].playingMidi.clear();
				if (!isPlaying) { playNote(60, 4, patterns[selectedPattern].instrument, selectedPattern, lastVolume, true); }
			}, 20);
		}
	}
	...
	colorPickerBox.add(colorPickerMenu);
	root.add(colorPickerBox);
	setupEventListeners();
	hI.oninput = (e) => {
		if (activeEl) {
			activeEl.caret = e.target.selectionStart;;
...
document.documentElement.onkeydown = function(e) {
	if ( activeInput == null && !activeInput) {
		if (e.key === " " || e.code === "Space" || e.keyCode === 32) {
			e.preventDefault();
			if(instrumentBox.style.opacity>0){
			instrumentOk.onClick()
			}
			if (view == "PianoRoll") {
				view = "DAW";
